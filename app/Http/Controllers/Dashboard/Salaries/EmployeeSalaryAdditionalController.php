...
use App\Exports\EmployeeSalaryAdditionalExport;
use App\Imports\EmployeeSalaryAdditionalImport;
use App\Http\Requests\Dashboard\Salaries\EmployeeSalaryAdditionalRequest;
...
class EmployeeSalaryAdditionalController extends Controller
{
    ...
    {
        return view('dashboard.salaries.additionals.create', compact('financeClnPeriod'));
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(EmployeeSalaryAdditionalRequest $request, $financeClnPeriodId)
    {
        try {
            $com_code = Auth::user()->com_code;
            $userId = Auth::user()->id;
            ...
            if ($financeClnPeriod->is_open != FinanceClnPeriodsIsOpen::Open) {
                return redirect()->route('dashboard.additionals.show', $financeClnPeriod->slug)->withErrors(['error' => 'عفوا الشهر المالى غير مفتوح !'])->withInput();
            }
            ...
            EmployeeSalaryAdditional::create($dataInsert);
            return redirect()->route('dashboard.additionals.show', $financeClnPeriod->slug)->with('success', 'تم أضاف الأضافى بنجاح');
        } catch (\Exception $e) {
            return redirect()
    ...
    public function showData(EmployeeSalaryAdditional $additional, FinanceClnPeriod $financeClnPeriod)
    {
        try {
            return view('dashboard.salaries.additionals.show_data', compact('additional', 'financeClnPeriod'));
        } catch (\Exception $e) {
            return redirect()
                ->route('dashboard.additionals.index')
                ->withErrors(['error' => $e->getMessage()]);
    ...
    public function edit(EmployeeSalaryAdditional $additional, FinanceClnPeriod $financeClnPeriod)
    {
        try {
            return view('dashboard.salaries.additionals.edit', compact('additional', 'financeClnPeriod'));
        } catch (\Exception $e) {
            return redirect()
                ->route('dashboard.additionals.index')
                ->withErrors(['error' => $e->getMessage()]);
     ...
     */
    public function update(EmployeeSalaryAdditionalRequest $request, EmployeeSalaryAdditional $additional)
    {
        try {
            $com_code = Auth::user()->com_code;
            $userId = Auth::user()->id;
            $financeClnPeriod = FinanceClnPeriod::where('com_code', $com_code)
                ->where('id', $additional->finance_cln_period_id)
                ->firstOrFail();
            if ($financeClnPeriod->is_open != FinanceClnPeriodsIsOpen::Open) {
                return redirect()->route('dashboard.additionals.show', $financeClnPeriod->slug)->withErrors(['error' => 'عفوا الشهر المالى غير مفتوح !'])->withInput();
            }
            ...
            return redirect()->route('dashboard.additionals.show', $financeClnPeriod->slug)->with('success', 'تم تعديل الأضافى بنجاح');
        } catch (\Exception $e) {
            return redirect()
                ->route('dashboard.additionals.index')
                ->withErrors(['error' => $e->getMessage()]);
     ...
     */
    public function destroy(EmployeeSalaryAdditional $additional, FinanceClnPeriod $financeClnPeriod)
    {
        try {
            $com_code = Auth::user()->com_code;
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            Excel::import(new EmployeeSalaryAdditionalImport(), $request->file('file'));
    ...
    public function print(Request $request)
    {
        try {
            ...
            // أو طباعة مباشرة
            return view('dashboard.salaries.additionals.partials.print', compact('additionals'));
        } catch (\Exception $e) {
            return redirect()
                ->route('dashboard.additionals.index')
                ->withErrors(['error' => $e->getMessage()]);

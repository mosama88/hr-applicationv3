...
class AdditionalTypeExport implements FromArray, WithHeadings, WithStyles
{
    protected $data;

    public function __construct($additionalTypes)
    {
        // نحضّر فقط الأعمدة المطلوبة
        $this->data = $additionalTypes->map(function ($additionalType) {
            return [
                'name'   => $additionalType->name,
            ];
        })->toArray();
    }
    ...
    {
        return ['نوع الاضافى']; // ترجم العناوين حسب الأعمدة المختارة
    }

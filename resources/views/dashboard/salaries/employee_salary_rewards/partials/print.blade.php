...
<body>
    <div class="header">
        ...
        <p>إجمالى الخصومات: {{ number_format($rewards->sum('total'), 2) }} جنيه</p>
        ...
        <tbody>
            @foreach ($rewards as $reward)
                <tr>
                    ...
                    <td>{{ $reward->mainSalaryEmployee->employee_name }}</td>
                    <td>{{ $reward->mainSalaryEmployee->department->name }}</td>
                    <td>{{ $reward->mainSalaryEmployee->branch->name }}</td>
                    <td>{{ $reward->additionalType->name }}</td>
                    <td>{{ $reward->total * 1 }}</td>
                    <td>{{ $reward->financeClnPeriod->year_and_month ?? 'غير معروف' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>

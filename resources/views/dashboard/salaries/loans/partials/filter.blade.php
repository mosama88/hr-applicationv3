...
<x-filter-component :otherInput="'total_loan'">
    <div class="mb-3 d-flex gap-2 mt-2">
        ...
        <form action="{{ route('dashboard.loans.export', $financeClnPeriod->slug) }}" method="GET">
            @csrf
        ...
        <form id="printForm" action="{{ route('dashboard.loans.print') }}" method="POST" target="_blank">
            @csrf
            <input type="hidden" name="employee_code_search" value="{{ request('employee_code_search') }}">
            <input type="hidden" name="name" value="{{ request('name') }}">
            <input type="hidden" name="department" value="{{ request('department') }}">
    ...
    <form action="{{ route('dashboard.loans.show', $financeClnPeriod->slug) }}" method="GET">
        <div class="row">

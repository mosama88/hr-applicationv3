    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">إستيراد ملف للبدلات المتغيرة لشهر
                    {{ $financeClnPeriod->slug }}</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('dashboard.employee_salary_allowances.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="alert alert-info" style="direction: rtl; text-align: right;">
                        ...
                        <ul>
                            <li>📄 يجب أن يكون الملف Excel بصيغة: <code>.xlsx</code> أو <code>.xls</code> أو
                                <code>.csv</code>.</li>
                            <li>✅ تأكد من أن الأعمدة مرتبة كما يلي: الاسم، البريد الإلكتروني، كلمة المرور.</li>
